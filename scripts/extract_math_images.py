#!/usr/bin/env python3
"""수학 문제 PDF → 각 문제 영역 PNG 저장 (data/images/math/)"""
    ...
    for page_idx, page in enumerate(doc):
        for block in page.get_text("dict")["blocks"]:
            ...
            for line in block["lines"]:
                ...
                if m:
                    q_num = int(m.group(1))
                    if 1 <= q_num <= 20 and q_num not in positions:
    ...
    for page_idx, page in enumerate(doc):
        ...
        for col in ("left", "right"):
            ...
            for i, (q_num, y_start) in enumerate(col_qs):
                y_end = col_qs[i + 1][1] - 4 if i + 1 < len(col_qs) else col_end_y
...
def main() -> int:
    ...
    for pdf_path in pdf_files:
        ...
        status = "✓" if q_count == 20 else "⚠"
        print(f"  {status} {exam_key}: 문제 {q_count}개 저장")
        total_q += q_count

    print(f"\n총 문제 {total_q}/320개 이미지 저장 → {IMAGE_DIR}")
    return total_q
