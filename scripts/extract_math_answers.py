#!/usr/bin/env python3
"""수학 정답표 PDF → data/math_answers.json"""
...
PDF_PATH = BASE_DIR / "data/pdfs/math/수학 정답표.pdf"
OUTPUT_PATH = BASE_DIR / "data/math_answers.json"

CIRCLE_MAP = {"①": 1, "②": 2, "③": 3, "④": 4}
CUSTOM_CIRCLE_MAP = {"\u085a": 1, "\u085b": 2, "\u085c": 3, "\u085d": 4}

HARDCODED_ANSWERS: dict[str, list[int]] = {
    "2020_1": [4, 1, 1, 3, 4, 3, 2, 3, 3, 2, 1, 4, 1, 4, 1, 2, 4, 3, 2, 1],
}
...
def parse_answers_standard(text: str) -> list:
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d+$", line):
            q_num = int(line)
            if 1 <= q_num <= 20 and i + 1 < len(lines):
                ...
                continue
        i += 1
    return [answers.get(n) for n in range(1, 21)]
        ...
        for line in block["lines"]:
            for span in line["spans"]:
                ...
                if 15 <= size <= 18:
                    ...
                    ans = CUSTOM_CIRCLE_MAP.get(text) or CUSTOM_CIRCLE_MAP.get(text[0])
                    if ans is not None:
...
def main():
    ...
    for page_idx in range(len(doc)):
        ...
        print(f"  {key}: {20 - none_count}/20개 정답 추출")
