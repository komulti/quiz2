#!/usr/bin/env python3
"""영어 정답표 PDF → data/en_answers.json"""
...
PDF_PATH = BASE_DIR / "data/pdfs/english/영어 정답표.pdf"
OUTPUT_PATH = BASE_DIR / "data/en_answers.json"

CIRCLE_MAP = {"①": 1, "②": 2, "③": 3, "④": 4}
CUSTOM_CIRCLE_MAP = {"\u085a": 1, "\u085b": 2, "\u085c": 3, "\u085d": 4}

# 벡터 전용 페이지 - 이미지로 직접 확인 후 하드코딩
HARDCODED_ANSWERS: dict[str, list[int]] = {
    "2020_1": [2, 4, 3, 4, 3, 1, 1, 1, 2, 2, 2, 3, 1, 2, 4, 4, 3, 2, 1, 4, 1, 4, 3, 3, 4],
}
        ...
        for line in block["lines"]:
            for span in line["spans"]:
                ...
                if 15 <= size <= 18:
                    ...
                    # 일부 스팬에 \x01이 붙는 경우가 있어 첫 글자만 사용
